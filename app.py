...
def generate_ringkasan_indikator(tahun, bulan, minggu_ke):
    ...
    try:
        ...
        return "Error membaca data ringkasan IPH."
...
if st.session_state.user_role == "Publik_Shared":
    ...
    if data:
        ...
        df_melt = df_top.melt(id_vars=['Komoditas'], value_vars=[str(th) for th in tahun_andil_list], var_name='Tahun', value_name='Frekuensi')
...
if (st.session_state.user_role in ["Admin", "Pegawai"]) and menu == "Input Rekap IPH":
    ...
    with tab1:
        ...
        with col2:
            kom2 = st.selectbox("Komoditas 2", komoditas_master, index=komoditas_master.index(default_kom2), key="kom2_sel")
        with col3:
            kom3 = st.selectbox("Komoditas 3", komoditas_master, index=komoditas_master.index(default_kom3), key="kom3_sel")
        fluk_kom = st.selectbox("Komoditas Fluktuasi", komoditas_master, index=komoditas_master.index(default_fluk), key="fluk_kom_sel")
        with st.form(key=f"iph_form_{st.session_state.form_key}"):
            ...
            with col_t2:
                bulan = st.selectbox("Bulan", range(1,13), format_func=lambda x: ['Jan','Feb','Mar','Apr','Mei','Jun','Jul','Agu','Sep','Okt','Nov','Des'][x-1], index=datetime.now().month-1, key="bulan_input")
            with col_t3:
                minggu = st.number_input("Minggu ke-", min_value=1, max_value=5, value=1, key="minggu_input")
            indikator_str = st.text_input("Indikator Perubahan Harga (%)", value="0", key="indikator_input")
            col_n1, col_n2, col_n3 = st.columns(3)
            with col_n1:
                nilai1_str = st.text_input(f"Nilai {kom1} (%)", value="0", key=f"nilai1_{st.session_state.form_key}")
            with col_n2:
                nilai2_str = st.text_input(f"Nilai {kom2} (%)", value="0", key=f"nilai2_{st.session_state.form_key}")
            with col_n3:
                nilai3_str = st.text_input(f"Nilai {kom3} (%)", value="0", key=f"nilai3_{st.session_state.form_key}")
